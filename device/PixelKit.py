...
Vendored from https://github.com/murilopolese/kano-pixel-kit-pixel32
(python/PixelKit.py) to run on top of vanilla, officially-built
MicroPython instead of Kano's own unmaintained firmware fork - see the
README for why. Two changes from upstream, both needed because this
runs on a current MicroPython build rather than Kano's 2018-era one:

1. `button_reset` referenced an undefined `BUTTON_NONE_PIN` constant
   (a bug in the original that would raise NameError on import); fixed
   to use `BUTTON_RESET` below.
2. `np.timing = 1` was set as a post-construction attribute (a 2018-era
   pattern). On current neopixel.py this clobbers internal state
   `write()` relies on, raising "object 'int' isn't a tuple or list".
   Fixed to pass `timing=1` as a constructor argument instead.
'''
...
np = NeoPixel(neopixel_pin, SIZE, timing=1)
